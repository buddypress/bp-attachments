...
function bp_attachments_prepare_avatar_for_js( $attachment = null ) {
	...
	$response = array(
		...
		'filename'  => wp_basename( $attachment->dir ),
		'url'       => $attachment->url,
		'path'      => $attachment->dir,
	...
	return apply_filters( 'bp_attachments_prepare_avatar_for_js', $response, $attachment );
}
...
function bp_attachments_handle_avatar_upload( $uploaded_file, $component, $item_id, $no_js = false ) {
	...
	if ( bp_is_active( 'groups' ) && $component == $bp->groups->id && empty( $bp->groups->current_group ) ) {
		$bp->groups->current_group = new stdClass();
		$bp->groups->current_group->id = $item_id;
	}

	require_once( ABSPATH . '/wp-admin/includes/file.php' );

	$uploadErrors = array(
		0 => __( 'The image was uploaded successfully', 'bp-attachments' ),
		1 => __( 'The image exceeds the maximum allowed file size of: ', 'bp-attachments' ) . size_format( bp_core_avatar_original_max_filesize() ),
		2 => __( 'The image exceeds the maximum allowed file size of: ', 'bp-attachments' ) . size_format( bp_core_avatar_original_max_filesize() ),
		3 => __( 'The uploaded file was only partially uploaded.', 'bp-attachments' ),
		4 => __( 'The image was not uploaded.', 'bp-attachments' ),
		6 => __( 'Missing a temporary folder.', 'bp-attachments' )
	);

	if ( ! bp_core_check_avatar_upload( $file ) ) {
		return new WP_Error( 'upload_error', sprintf( __( 'Your upload failed, please try again. Error was: %s', 'bp-attachments' ), $uploadErrors[$file['file']['error']] ), $file );
	}

	if ( ! bp_core_check_avatar_size( $file ) ) {
		return new WP_Error( 'upload_error', sprintf( __( 'The file you uploaded is too big. Please upload a file under %s', 'bp-attachments' ), size_format( bp_core_avatar_original_max_filesize() ) ), $file );
	}

	if ( ! bp_core_check_avatar_type( $file ) ) {
		return new WP_Error( 'upload_error', __( 'Please upload only JPG, GIF or PNG photos.', 'bp-attachments' ), $file );
	}

	// Filter the upload location
	add_filter( 'upload_dir', "{$component}_avatar_upload_dir", 10, 0 );

	$bp->attachments->original = wp_handle_upload( $file['file'], array( 'action'=> 'bp_attachments_upload_avatar' ) );

	// Remove the upload_dir filter, so that other upload URLs on the page
	// don't break
	remove_filter( 'upload_dir',"{$component}_avatar_upload_dir", 10, 0 );

	// Move the file to the correct upload location.
	if ( ! empty( $bp->attachments->original['error'] ) ) {
		return new WP_Error( 'upload_error', sprintf( __( 'Upload Failed! Error was: %s', 'bp-attachments' ), $bp->attachments->original['error'] ), $file );
	}

	// Get image size
	$size  = @getimagesize( $bp->attachments->original['file'] );
	$error = false;

	// Check image size and shrink if too large
	if ( $size[0] > bp_core_avatar_original_max_width() ) {
		$editor = wp_get_image_editor( $bp->attachments->original['file'] );
	...
	if ( ! isset( $bp->attachments->image ) )
		$bp->attachments->image = new stdClass();

	$bp->attachments->image->item_id = $item_id;
	$bp->attachments->image->component = $component;

	// We only want to handle one image after resize.
	if ( empty( $bp->attachments->resized ) ) {
		$bp->attachments->image->dir = str_replace( bp_core_avatar_upload_path(), '', $bp->attachments->original['file'] );
	} else {
		$size  = @getimagesize( $bp->attachments->resized['path'] );
		$bp->attachments->image->dir = str_replace( bp_core_avatar_upload_path(), '', $bp->attachments->resized['path'] );
		@unlink( $bp->attachments->original['file'] );
	}

	// Check for WP_Error on what should be an image
	if ( is_wp_error( $bp->attachments->image->dir ) ) {
		return $bp->attachments->image->dir;
	}
	...
	$bp->attachments->image->name = $name;

	$bp->attachments->image->width = $size[0];
	$bp->attachments->image->height = $size[1];
