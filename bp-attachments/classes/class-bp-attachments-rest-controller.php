...
class BP_Attachments_REST_Controller extends WP_REST_Attachments_Controller {
	...
	public function register_routes() {
		...
		register_rest_route(
			$this->namespace,
			'/' . $this->rest_base . '/members/(?P<user_id>[\d]+)/profile-image',
			array(
				'args'   => array(
					'user_id' => array(
				...
				array(
					'methods'             => WP_REST_Server::CREATABLE,
					'callback'            => array( $this, 'create_profile_image' ),
					'permission_callback' => array( $this, 'create_profile_image_permissions_check' ),
				),
				'schema' => array( $this, 'get_profile_image_schema' ),
			)
		);
	}
	...
	/**
	 * Check if the current user can change a member's profile image.
	 *
	 * @since 1.0.0
	 *
	 * @param WP_REST_Request $request Full data about the request.
	 ...
	 * @return WP_Error|WP_REST_Response Response object on success, WP_Error object on failure.
	 */
	public function create_profile_image( $request ) {
		$request->set_param( 'context', 'edit' );
		...
		$schema = $this->get_profile_image_schema();
		$data   = array();
	...
	/**
	 * Get the member's profile image schema, conforming to JSON Schema.
	 *
	 * @since 1.0.0
	 *
	 * @return array
	 */
	public function get_profile_image_schema() {
		/**
		 * Filters the attachments profile image schema.
		 *
		 * @since 1.0.0
		 *
